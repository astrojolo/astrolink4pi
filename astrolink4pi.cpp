...
bool AstroLink4Pi::Connect()
{
  ...
  DEBUGF(INDI::Logger::DBG_SESSION, "Connected on %s (%s), kernel %s", m_SystemInfo.getHostname().c_str(),
         m_SystemInfo.getModel().c_str(), m_SystemInfo.getKernelVersion().c_str());

  // Optional modules
  if (m_BoardIO.revision() >= 4 && !m_PowerMonitor.open())
  {
    DEBUG(INDI::Logger::DBG_DEBUG, "Power monitor not detected.");
  }
  if (m_BoardIO.revision() >= 3 && !m_SHTReader.open())
  {
    DEBUG(INDI::Logger::DBG_DEBUG, "SHT sensor not detected.");
  }
  if (m_BoardIO.revision() >= 3 && !m_MLXReader.open())
  {
    DEBUG(INDI::Logger::DBG_DEBUG, "MLX sensor not detected.");
  }
  if (m_BoardIO.revision() >= 3 && !m_TSLReader.open())
  {
  ...
  // read last position from file & convert from MAX_RESOLUTION to current
  // resolution
...
bool AstroLink4Pi::Disconnect()
{
  ...
  if (m_BoardIO.revision() >= 4) m_PowerMonitor.close();
...
bool AstroLink4Pi::initProperties()
{
  ...
  FI::SetCapability(FOCUSER_CAN_ABS_MOVE | FOCUSER_CAN_REL_MOVE | FOCUSER_CAN_REVERSE | FOCUSER_CAN_SYNC |
                    FOCUSER_CAN_ABORT | FOCUSER_HAS_BACKLASH);
  ...
  IUFillSwitchVector(&FocusResolutionSP, FocusResolutionS, 6, getDeviceName(), "FOCUS_RESOLUTION", "Resolution",
  ...
  IUFillSwitchVector(&FocusHoldSP, FocusHoldS, 6, getDeviceName(), "FOCUS_HOLD", "Hold power", OPTIONS_TAB, IP_RW,
                     ISR_1OFMANY, 0, IPS_IDLE);

  // Step delay setting
  IUFillNumber(&FocusStepDelayN[0], "FOCUS_STEPDELAY_VALUE", "microseconds", "%0.0f", 200, 20000, 1, 2000);
  IUFillNumberVector(&FocusStepDelayNP, FocusStepDelayN, 1, getDeviceName(), "FOCUS_STEPDELAY", "Step Delay",
                     OPTIONS_TAB, IP_RW, 0, IPS_IDLE);

  // Focuser temperature
  IUFillNumber(&FocusTemperatureN[0], "FOCUS_TEMPERATURE_VALUE", "°C", "%0.2f", -50, 50, 1, 0);
  IUFillNumberVector(&FocusTemperatureNP, FocusTemperatureN, 1, getDeviceName(), "FOCUS_TEMPERATURE", "Temperature",
                     MAIN_CONTROL_TAB, IP_RO, 0, IPS_IDLE);

  // Temperature Coefficient
  IUFillNumber(&TemperatureCoefN[0], "steps/C", "", "%.1f", -1000, 1000, 1, 0);
  IUFillNumberVector(&TemperatureCoefNP, TemperatureCoefN, 1, getDeviceName(), "Temperature Coefficient", "",
                     OPTIONS_TAB, IP_RW, 0, IPS_IDLE);
  ...
  IUFillNumberVector(&FocuserInfoNP, FocuserInfoN, 3, getDeviceName(), "FOCUSER_PARAMETERS", "Focuser Info",
                     MAIN_CONTROL_TAB, IP_RO, 0, IPS_IDLE);

  // Maximum focuser travel
  IUFillNumber(&FocuserTravelN[0], "FOCUSER_TRAVEL_VALUE", "mm", "%0.0f", 10, 200, 10, 10);
  IUFillNumberVector(&FocuserTravelNP, FocuserTravelN, 1, getDeviceName(), "FOCUSER_TRAVEL", "Max Travel", OPTIONS_TAB,
                     IP_RW, 0, IPS_IDLE);
  ...
  IUFillNumberVector(&ScopeParametersNP, ScopeParametersN, 2, getDeviceName(), "TELESCOPE_INFO", "Scope Properties",
  ...
  IUFillTextVector(&SysTimeTP, SysTimeT, 2, getDeviceName(), "SYSTEM_TIME", "System Time", SYSTEM_TAB, IP_RO, 60,
                   IPS_IDLE);
  ...
  IUFillTextVector(&SysInfoTP, SysInfoT, 6, getDeviceName(), "SYSTEM_INFO", "System Info", SYSTEM_TAB, IP_RO, 60,
                   IPS_IDLE);

  IUFillNumber(&FanPowerN[0], "FAN_PWR", "Speed [%]", "%0.0f", 0, 100, 1, 33);
  IUFillNumberVector(&FanPowerNP, FanPowerN, 1, getDeviceName(), "FAN_POWER", "Internal fan", SYSTEM_TAB, IP_RO, 60,
                     IPS_IDLE);
  ...
  IUFillTextVector(&RelayLabelsTP, RelayLabelsT, 4, getDeviceName(), "RELAYLABELS", "Relay Labels", OPTIONS_TAB, IP_RW,
                   60, IPS_IDLE);

  IUFillNumber(&SQMOffsetN[0], "SQMOffset", "mag/arcsec2", "%0.2f", -1, 1, 0.01, 0);
  IUFillNumberVector(&SQMOffsetNP, SQMOffsetN, 1, getDeviceName(), "SQMOFFSET", "SQM calibration", OPTIONS_TAB, IP_RW,
                     60, IPS_IDLE);
  ...
  IUFillNumberVector(&StepperCurrentNP, StepperCurrentN, 1, getDeviceName(), "STEPPER_CURRENT", "Stepper current",
                     OPTIONS_TAB, IP_RW, 0, IPS_IDLE);

  IUFillSwitch(&Switch1S[S1_ON], "S1_ON", "ON", ISS_OFF);
  IUFillSwitch(&Switch1S[S1_OFF], "S1_OFF", "OFF", ISS_ON);
  IUFillSwitchVector(&Switch1SP, Switch1S, 2, getDeviceName(), "SWITCH_1", RelayLabelsT[0].text, OUTPUTS_TAB, IP_RW,
                     ISR_1OFMANY, 0, IPS_IDLE);
  ...
  IUFillNumberVector(&PowerReadingsNP, PowerReadingsN, 6, getDeviceName(), "POWER_READINGS", "Power readings",
                     OUTPUTS_TAB, IP_RO, 60, IPS_IDLE);
...
bool AstroLink4Pi::ISNewNumber(const char* dev, const char* name, double values[], char* names[], int n)
{
  ...
  if (!strcmp(dev, getDeviceName()))
  {
    ...
    if (!strcmp(name, ScopeParametersNP.name))
    {
      ...
      getFocuserInfo();
      DEBUGF(INDI::Logger::DBG_SESSION, "Scope parameters set to %0.0f / %0.0f.", ScopeParametersN[SCOPE_DIAM].value,
             ScopeParametersN[SCOPE_FL].value);
      return true;
    }
    ...
    if (strstr(name, "FOCUS_")) return FI::processNumber(dev, name, values, names, n);
    if (strstr(name, "WEATHER_")) return WI::processNumber(dev, name, values, names, n);
  }
...
//        return true;
    ...
    if (!strcmp(name, FocusResolutionSP.name))
    {
      ...
      if (FocusResolutionS[RES_1].s == ISS_ON) resolution = 1;

      if (FocusResolutionS[RES_2].s == ISS_ON) resolution = 2;

      if (FocusResolutionS[RES_4].s == ISS_ON) resolution = 4;

      if (FocusResolutionS[RES_8].s == ISS_ON) resolution = 8;

      if (FocusResolutionS[RES_16].s == ISS_ON) resolution = 16;

      if (FocusResolutionS[RES_32].s == ISS_ON) resolution = 32;
    ...
    if (strstr(name, "FOCUS")) return FI::processSwitch(dev, name, states, names, n);
    if (strstr(name, "WEATHER_")) return WI::processSwitch(dev, name, states, names, n);
  }
  ...
  if (!strcmp(dev, getDeviceName()))
  {
    ...
    if (!strcmp(name, RelayLabelsTP.name))
    {
      ...
      DEBUG(INDI::Logger::DBG_SESSION,
            "AstroLink 4 Pi labels set . You need to save configuration and "
            "restart driver to activate the changes.");
      DEBUGF(INDI::Logger::DBG_DEBUG, "AstroLink 4 Pi labels set to OUT1: %s, OUT2: %s, PWM1: %s, PWM2: %s",
             RelayLabelsT[0].text, RelayLabelsT[1].text, RelayLabelsT[2].text, RelayLabelsT[3].text);
...
void AstroLink4Pi::TimerHit()
{
  ...
  if (m_BoardIO.revision() >= 3) readTSL();
  if (m_BoardIO.revision() >= 4) readPower();
  focuserUpdate();
  ...
  if (nextSystemRead < timeMillis)
  {
    ...
    switch (m_Cycle)
    {
      case SensorCycle::SHT_T:
        if (m_BoardIO.revision() >= 3) readSHT();
        break;
      case SensorCycle::SHT_R:
        if (m_BoardIO.revision() >= 3) readSHT();
        break;
      case SensorCycle::MLX:
        if (m_BoardIO.revision() >= 3) readMLX();
        break;
...
bool AstroLink4Pi::AbortFocuser() { return m_Focuser.abortFocuser(); }
...
IPState AstroLink4Pi::MoveRelFocuser(FocusDirection dir, uint32_t ticks)
{
  ...
  if (target > (int32_t)FocusAbsPosNP[0].getMax()) target = (int32_t)FocusAbsPosNP[0].getMax();
...
void AstroLink4Pi::getFocuserInfo()
{
  ...
  DEBUGF(INDI::Logger::DBG_DEBUG, "Focuser Info: %0.2f %0.2f %0.2f.", FocuserInfoN[0].value, FocuserInfoN[1].value,
         FocuserInfoN[2].value);
}
